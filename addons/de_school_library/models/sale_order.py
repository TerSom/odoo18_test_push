...
class CirculationAgreement(models.Model):
    ...
    def _compute_is_book_late(self):
        ...
        for order in self:
            # Ensure `is_late` is always assigned a value
            order.is_book_late = False  # Default to False
    
            # Only compute if conditions are met
            if order.is_borrow_order and order.borrow_status in ['issue', 'return'] and order.borrow_next_action_date:
                order.is_book_late = order.borrow_next_action_date + tolerance_delay < now
